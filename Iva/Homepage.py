from flask import g, Flask, session, redirect, request
import sqlite3
import hashlib
import json
...
<nav class="navbar navbar-inverse">
	...
	if is_logged_in:
		...
		<ul class="nav navbar-nav">
			...
			<li class=""><a href="/articlesList">Новини</a></li>
...
def article(article_id):
	if not is_logged() and article_id == 0:
...
@app.route("/articlesList", methods=['GET'])
def all_articles():
	if not is_logged():
		return redirect("/", code=401)
	return """
				...
				container.on('load.infiniteScroll', function(event, data) {
					if(data.substr(0, 3) == "OK:") {
						...
						data.forEach(function(element) {
							...
							result += '</div>';
							result += '<div class="panel-body">' + element.body.substr(0, 50) + '...';
							result += '<div><a href="/article/' + element.id + '">Прочети повече</a></div>';
							result += '</div></div>';
						});
...
<html>
	...
	<body>
		...
		<div class="container">
			<hr />
			<h2>Здравословен начин на живот с промяна на хранителните навици</h2>
			<p>
				Може би решението е било дълго обмисляно и променяно многократно, 
				а може и тази статия да ви накара да погледнете с нови очи на здравето си. 
				Смисълът на всичко, което правим, е да ви накараме да се чувствате по-добре и да бъдете здрави.
			...
			<h3>Кои са лошите навици, които пречат на вашия здравословен начин на живот</h3>
			<p>
				Безопасността на храната е много важна за вашия здравословен начин на живот, тъй като всяка 
				година много хора заболяват от хранителни инфекции или интоксикации. Има методи за прочистване на 
				организма или детоксикация, които биха помогнали да се отървете от вредните елементи.
				Установено е, че за последните 15 години са се оформили няколко отрицателни тенденции, които се отнасят до 
				здравето на населението в България. Това са:
				<ol>
					<li>Ниска консумация на риба, сурови плодове и зеленчуци през зимата и пролетта.</li>
					<li> Ниска консумация на пълнозърнести продукти.</li>
					<li> Липсва на витамини и минерали, необходими на организма за здравословен начин на живот.</li>
					<li> Увеличен прием на джънк храни, животински мазнини и храни, съдържащи захар.</li>
				</ol>
			</p>
		</div>
	</body>
...
<html>
	...
	<body>
		...
		<div class="container">
			<hr>
			<h2>10 тайни за хидратация на тялото</h2>
			<div padding: 16px;> 
				Кожата е най-големият орган от човешкото тяло, съдържащ над 70 % вода. Тази вода й 
				е необходима не само, за да функционира пълноценно като защитен механизъм на организма, 
				но и за да изглежда добре.
				...
				в норма – както вътрешно, така й външно.
				<ol>
					<li>Пийте минимум 2 л. вода дневно. Дехидратацията на организма 
					проличава най-напред върху кожата - тя губи цвят, блясък и еластичност. За свежест и 
					сияен тен започвайте
					деня с чаша вода (сънят също дехидратира), старайте се да пиете по 200 мл вода на всеки 
					час през деня, дори 
					до чашата с кафе поставяйте чаша вода.</li>
					 <li>Хранете се здравословно и балансирано. Консумирането на сочни пресни плодове и 
					зеленчуци, е 
					чудесен начин
					за вътрешна хидратация. Препоръчваме ви диня, грейпфрут, краставици, целина, марули, спанак, ягоди, 
					тиквички и 
					домати, които освен ценни витамини и минерали, съдържат над 90 % вода.</li>
					<li>Ограничете вредните навици. Цигарите, алкохолът и недоспиването изсушават кожата, а подсладените
					газирани 
					напитки, кафето и чаят действат диуретично, т.е. оставят организма без течности. Затова контролирайте 
					приема им 
					и ги заменете с нещо по-полезно.</li>
					<li>Защитавайте кожата от околната среда. Слънцето, свободните радикали и екстремните температури 
					увреждат кожата
					и я дехидратират. Затова е добре да ограничите контакта с тях и да я защитавате с козметични продукти, 
					съдържащи витамин С, антиоксиданти, фотозащитен фактор и хидратиращи съставки.</li>
					<li>Къпете се бързо и с хладка вода. Продължителните горещи душове отмиват естествените мазнини и 
					предпазния 
					слой, задържащ влагата. За да намалите този процес, заъвършвайте къпането със студен душ.</li>
					<li>Внимавайте със сапуна. Много сапуни съдържат съставки, които допълнително изсушават кожата. Затова 
					подбирайте
					такива, обогатени с природни масла (от ший, нар, лимонова трева, бадемово масло, растителни екстракти) и 
					образуващи нежна почистваща пяна, или душ - гелове с хидратираща формула.</li>
					<li>Попивайте кожата нежно и не напълно. Интензивното търкане с кърпата уврежда епидермиса и сваля 
					последните 
					защитни мазнини. По-добре попивайте тялото повърхностно с леки порупвания и оставяйте да изсъхне от 
					самосебе 
					си.</li>
					<li>Хидратирайте. За да запечатате абсорбираната в клетките вода от душа, веднага след баня 
					нанасяйте 
					хидратиращ
					продукт за тяло върху още влажна кожа. Съсредоточете се върху склонните към изсушаване зони като колене, 
					лакти,гърди и корем. Тази процедура е особено важна след излагане на слънце.</li>
					<li>Ексфолирайте. Отстраняването на мъртвите клетки от кожата на тялото веднъж седмично позволява на 
					водата и
					хидратиращите продукти да проникнат в дълбочина и да се запазят по-дълго. През останалите дни 
					използвайте гъба
					луфа за изглаждане на епидермиса и стимулиране на кръвообръщението.</li>
					<li>Подбирайте продуктите разумно. Съобразяайте текстурата (лосион, мляко или крем за тяло) според 
					кожата си. 
					Добре е формулите да са на водна основа, да съдържат хидратиращи природни съставки (жожоба, семена от 
					грозде, 
					какао, цитруси) и хиалуронова киселина. Сменяйте продуктите през сезоните, тъй като през зимата кожата 
					се нуждае
					от повече хидратация.</li>
				</ol>	 
				Основната тайна за красива и хидратирана кожа е постоянството. Спазвайте стриктно горните съвети, 
				спортувайте 
				редовно и спете минимум 8 часа на нощ. Така ще снабдявате тялото си с нужното количество вода и жизнени 
				сили,
				за да се справя с ежедневните стресови за организма фактори. 
			</div>
			<hr>
		</div>
	</body>
